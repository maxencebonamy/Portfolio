...
const Navbar = ({ toggleTheme, lang, toggleLang, width }: NavbarProps) => {
    ...
    return (
        <nav className="fixed w-full z-20 h-32 flex items-center justify-between px-12 gradient-bg">
            ...
            <ul
                style={ulStyle}
                onMouseEnter={width >= BREAKPOINTS.lg ? () => setIsOpen(true) : () => {}}
                onMouseLeave={width >= BREAKPOINTS.lg ? () => setIsOpen(false) : () => {}}
            ...
            >
                ...
                <span
                    className={cn(
                        "absolute transition-all duration-500 h-[0.15rem] w-12 bg-dark dark:bg-light rounded-full top-[2.925rem] right-12",
                        isOpen ? "-rotate-45 translate-y-4" : "")}
                />
                <span
                    style={isOpen ? openSpanStyle : {}}
                    className="absolute transition-all duration-500 h-[0.15rem] w-12 bg-dark dark:bg-light rounded-full top-[3.925rem] right-12"
                />
                <span
                    className={cn(
                        "absolute transition-all duration-500 h-[0.15rem] w-12 bg-dark dark:bg-light rounded-full top-[4.925rem] right-12",
                        isOpen ? "rotate-45 -translate-y-4" : "")}
                />

                {
                    ...
                    <span
                        ...
                        className="absolute h-12 w-12 top-10 right-12"
                    /> :
                    <></>
                }
            </ul>
